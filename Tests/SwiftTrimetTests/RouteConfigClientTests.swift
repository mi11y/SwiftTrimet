...
class RouteConfigClientTests: XCTestCase {
    ...
    func testFetchRoutesOnSuccess() {
        ...
        guard let expectedJSON = try? JSON(data: TestData.exampleJSON.data) else { XCTFail("Failed to parse test JSON"); return }
    ...
    func testSetQueryParameters() {
        ...
        guard let expectedJSON = try? JSON(data: TestData.exampleJSON.data) else {
    ...
    private struct MockConfigration {
        var statusCode = 200
        var payload: Data = TestData.exampleJSON.data
    ...
    private func initQueryParameters() -> RouteConfigClient.RoutesQueryParameters {
        var newParameters = RouteConfigClient.RoutesQueryParameters()
